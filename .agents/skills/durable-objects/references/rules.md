...
Create one DO per logical unit needing coordination: chat room, game session,
...
For hierarchical data, create separate child DOs. Parent tracks references,
children handle own state.
...
Available hints: `wnam`, `enam`, `sam`, `weur`, `eeur`, `apac`, `oc`, `afr`,
`me`.
...
**Note:** `PRAGMA user_version` is **not supported** in Durable Objects SQLite
storage. Use a `_sql_schema_migrations` table instead:
...
— provides a `SQLSchemaMigrations` class that tracks executed migrations both in
memory and in storage. Also see
...
— a reference implementation of the same pattern used by the Cloudflare Actors
framework.
...
**Rule**: Always persist critical state to SQLite first, then update in-memory
cache.
...
Storage operations automatically block other requests (input gates). Responses
wait for writes (output gates).
...
**Note**: Uncaught exceptions may terminate the DO instance. In-memory state is
lost, but SQLite storage persists.
