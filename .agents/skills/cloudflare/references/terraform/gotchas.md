...
Some resources have known state drift. Add lifecycle blocks to prevent perpetual
diffs:
...
Provider v5 is current (auto-generated from OpenAPI). v4→v5 has breaking
changes:
...
**Solution:** Use `WNAM`, `ENAM`, `WEUR`, `EEUR`, `APAC` (not `wnam`, `enam`,
etc.)
...
**Solution:** Choose one deployment method; if using Terraform, remove wrangler
deployments
...
**Solution:** Set `CLOUDFLARE_API_TOKEN` environment variable or check token
permissions in dashboard

### "State locking errors"

**Cause:** Multiple concurrent Terraform runs or stale lock from crashed
process  
**Solution:** Remove stale lock with `terraform force-unlock <lock-id>` (use
with caution)
