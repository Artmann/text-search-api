...
Recommended approach inside Workers. No HTTP overhead, auth handled
automatically.
...
Pre-fetches flags on init, then evaluates synchronously. Only `prefetchFlags`
are available.
...
For managing flags via the REST API (create, update, delete), set these
...
Base URL:
`https://api.cloudflare.com/client/v4/accounts/$CLOUDFLARE_ACCOUNT_ID/flagship`
...
Flagship bindings work in local dev with `wrangler dev`. Flag evaluation uses
the live Flagship configuration — there is no local flag store. Ensure the
`app_id` in your Wrangler config points to a valid app.
