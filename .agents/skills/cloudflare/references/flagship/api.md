...
The binding is available as `env.FLAGS` (type `Flagship` from
...
**Important:** Only flags listed in `prefetchFlags` are available. Unlisted
...
Before making any REST API calls (create, read, update, delete, toggle flags),
...
Requires an API token with `flagship:evaluate` permission. Context attributes
passed as query params. Returns:
...
- `priority`: integer >= 1, unique across rules in the flag (lower = evaluated
  first)
- `conditions`: array of base or logical conditions
- `serve_variation`: must be a key in `variations`
- `rollout`: optional. `percentage` 0-100. `attribute` defaults to
  `targetingKey`.
