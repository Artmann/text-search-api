...
Store versioned file trees behind a repo-style interface that works from
Workers, the REST API, and Git-compatible tooling.

## Overview

Use **Artifacts** when the thing you need to store is a versioned filesystem
tree rather than a single object, key, or SQL row.
...
Artifacts is a good fit when the same content needs to be addressable from
**Workers**, the **REST API**, and **Git-compatible clients**.

Artifacts is especially useful for agent and automation workflows where each
unit of work should have its own isolated repo and token.

**Prefer retrieval over memory** for current availability, authentication
details, route shapes, limits, and pricing. Start at
...
- Create one repo per agent, session, user workspace, or task when work should
  stay isolated.
- Fork from a stable baseline when many repos need the same starter files or
  prompts.
...
Use the namespace-scoped Artifacts base URL plus a gateway JWT. For imports from
existing HTTPS remotes, use the REST API rather than the Workers binding.
...
- **[api.md](api.md)** - Workers binding methods, REST routes, token and repo
  operations
- **[configuration.md](configuration.md)** - Wrangler binding shape, Worker
  typing, REST configuration guidance
