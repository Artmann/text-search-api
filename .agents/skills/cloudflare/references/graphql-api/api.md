...
The schema has a single entry point: `Query.viewer`. Mutations are not
supported.
...
Aggregated dataset nodes (`*Groups`) return these field categories. Not every
node has all — use introspection to check.

### count

Total events in the group. Available on `*Groups` nodes but **not** on raw
`*Adaptive` nodes (e.g., `workersInvocationsAdaptive` — use `sum { requests }`
instead).
...
Percentile distributions (on datasets like `workersInvocationsAdaptive`).
Available percentiles: P25, P50, P75, P90, P95, P99, P999 for `cpuTime`,
`wallTime`, `requestDuration`, `duration`, `responseBodySize`.
...
`scriptName`, `scriptTag`, `scriptVersion`, `environmentName`, `status`,
`usageModel`, `coloCode`, `dispatchNamespaceName`, `isDispatcher`

### Firewall Dimensions (firewallEventsAdaptive)

`action`, `source`, `ruleId`, `clientCountryName`, `clientIP`, `clientAsn`,
`userAgent`
...
**Always include a time range filter.** Multiple filters at the same level are
implicitly AND-ed.
...
> `_notin` and `_notlike` are in the schema but not in official docs. Confirmed
> via introspection.
...
Sort with `orderBy: [field_ASC]` or `[field_DESC]`. Multiple sort fields
supported.
