...
Filter by `scriptName` for a specific Worker. Add `datetimeFiveMinutes`
dimension + `orderBy: [datetimeFiveMinutes_ASC]` for error rate over time.
...
For aggregated firewall stats, use `firewallEventsAdaptiveGroups` with
`action: "block"` filter and group by `ruleId`, `source`, `datetimeHour`.
...
For cache hit ratio over time, use aliases to query the same dataset twice —
once with `cacheStatus: "hit"` filter and once without — then compute the ratio
client-side.
...
Both are account-scoped — nest under
`accounts(filter: { accountTag: $accountTag })`.

## Best Practices

**Always include time filters.** Queries without time filters scan all data and
are slow/expensive.
...
**Use aliases** for querying the same dataset with different filters in one
request.

**Request only needed fields.** Extra dimensions and metrics increase query
cost.
